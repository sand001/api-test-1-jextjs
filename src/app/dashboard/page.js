...
export default function ApiKeysDashboard() {
  ...
  useEffect(() => {
    // Aquí deberías hacer el fetch real de las API keys
    setApiKeys(dummyData);
  }, []);

  const handleCreateKey = async () => {
    if (!newKeyName.trim()) return;
    // Implementar la creación real de API key
    const newKey = {
      id: Date.now(),
      name: newKeyName,
  ...
  const handleDeleteKey = async (id) => {
    // Implementar la eliminación real de API key
                ...
                {apiKeys.map((key) => (
                  <tr key={key.id} className="border-b last:border-0">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <code className="text-sm font-mono">tvly-********************************</code>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => handleDeleteKey(key.id)} className="text-gray-400 hover:text-gray-600">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
